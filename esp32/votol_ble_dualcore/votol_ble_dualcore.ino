...
static const int FORCED_BATTERY_PERCENT = 55;        // Forced fallback battery % when CAN/BMS data is unavailable
...
std::atomic<float> atomicOdometerKm{0.0f};
std::atomic<VehicleMode> atomicMode{MODE_PARK};  // Thread-safe mode
std::atomic<bool> atomicRegenActive{false};
std::atomic<TransportMode> transportMode{TRANSPORT_BLE};  // Thread-safe transport mode

static uint8_t liveDataResponseBuffer[24] = {0};
static uint8_t liveDataResponseFrames = 0;
static bool liveDataResponseCollecting = false;
...
int valSOC = FORCED_BATTERY_PERCENT;
...
void handleCANMessage(twai_message_t &msg) {
  ...
  // Control live-data response (0x3FE): 3 x 8-byte frames containing status, voltage, current, RPM, etc.
  // Re-gen flag is carried in frame 2, byte 6 (B22 bit7), not in the raw 0x0A010810 mode byte.
  if (id == 0x3FE && msg.data_length_code == 8) {
    const uint8_t frame0Header[4] = {0x09, 0x55, 0xAA, 0xAA};

    if (memcmp(msg.data, frame0Header, sizeof(frame0Header)) == 0) {
      memset(liveDataResponseBuffer, 0, sizeof(liveDataResponseBuffer));
      liveDataResponseFrames = 0;
      liveDataResponseCollecting = true;
    }

    if (!liveDataResponseCollecting) {
      return;
    }

    memcpy(&liveDataResponseBuffer[liveDataResponseFrames * 8], msg.data, 8);
    liveDataResponseFrames++;

    if (liveDataResponseFrames >= 3) {
      uint8_t statusBits = liveDataResponseBuffer[22];
      atomicRegenActive.store((statusBits & 0x80) != 0, std::memory_order_release);
      liveDataResponseFrames = 0;
      liveDataResponseCollecting = false;
    }
  ...
  if (id == 0x0A010810 && msg.data_length_code >= 8) {
    ...
    uint8_t modeByte = m & 0x7F;
    VehicleMode newMode = MODE_PARK;  // Default for unknown modes
    bool regenBit = (m & 0x80) != 0;

    // Controller mode byte uses the legacy values already present in the project.
    // bit7 is reserved for regeneration flag, so we strip it before decoding the mode,
    // then capture the regen bit separately for the dashboard display.
    if (modeByte == 0x00) newMode = MODE_PARK;
    else if (modeByte == 0x61) newMode = MODE_CHARGING;
    else if (modeByte == 0x70) newMode = MODE_DRIVE;
    else if (modeByte == 0x50 || modeByte == 0xF0 || modeByte == 0x30 || modeByte == 0xF8) newMode = MODE_REVERSE;
    else if (modeByte == 0x72 || modeByte == 0xB2) newMode = MODE_BRAKE;
    else if (modeByte == 0xB0) newMode = MODE_SPORT;
    else if (modeByte == 0x78 || modeByte == 0x08) newMode = MODE_STAND;
    // Unknown modes default to MODE_PARK (already set above)

    atomicMode.store(newMode, std::memory_order_release);
    atomicRegenActive.store(regenBit, std::memory_order_release);
  ...
  // Votol controller speed + odometer frame (0x9026105A)
  // Example layout from the documentation: bytes 0..1 = odometer (little-endian, km), byte 5 = speed (km/h)
  // This sensor frame is commonly used as the source for odometer accumulation when 0x0A010810 is absent.
  if (id == 0x9026105A && msg.data_length_code >= 6) {
    uint16_t odoRaw = (uint16_t)(msg.data[0] | (msg.data[1] << 8));
    uint8_t speedRaw = msg.data[5];

    atomicSpeed.store((int)speedRaw, std::memory_order_release);
    atomicOdometerKm.store((float)odoRaw, std::memory_order_release);
  ...
  // Votol controller frame (0x90261022) - RPM, voltage, current
  if (id == 0x90261022 && msg.data_length_code >= 8) {
    uint16_t rpmRaw = (uint16_t)(msg.data[2] | (msg.data[3] << 8));
    uint16_t voltsRaw = (uint16_t)(msg.data[4] | (msg.data[5] << 8));
    int16_t ampsRaw = (int16_t)(msg.data[6] | (msg.data[7] << 8));

    int rpm = (int)rpmRaw;
    int speed = (int)round((float)rpm * 0.0783744f);

    atomicRPM.store(rpm, std::memory_order_release);
    atomicSpeed.store(speed, std::memory_order_release);

    float volts = voltsRaw / 100.0f;
    float amps = ampsRaw / 100.0f;
    float power = volts * amps;
    atomicVoltsRaw.store((int32_t)(volts * 10.0f), std::memory_order_release);
    atomicAmpereRaw.store((int32_t)(amps * 10.0f), std::memory_order_release);
...
void canTask(void *pvParameters) {
  ...
  while (true) {
    ...
    if (canAge > 3000UL) {
      canDataReady.store(false, std::memory_order_release);
      valSOC = FORCED_BATTERY_PERCENT;
    }
    ...
    // READ-ONLY MODE:
    // Keep TWAI in LISTEN_ONLY for the BMS/CAN bus and do not inject any charger messages.
    // This avoids disrupting the battery/BMS CAN traffic while still allowing monitoring.
...
void setup() {
  ...
  valSOC = preferences.getInt("soc", FORCED_BATTERY_PERCENT);
